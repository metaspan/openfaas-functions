...
async function getChainProperties() {
  var res = await axios.get(`${DOTASMA_API_BASE}/${CHAIN}/rpc/system/properties`)
...
async function getBalances(ids = [], batchSize=25) {
  ...
  for(var i = 0; i <= ids.length; i += batchSize) {
    ...
    //const res = await axios.get(`${DOTASMA_API_BASE}/${CHAIN}/query/system/accountMulti`, { params: { ids: batch }})
  ...
  console.log('acccounts[0] 123', accounts[0])
  const free = accounts.reduce((current, account, idx) => { return current + BigInt(account.data ? account.data.free : 0) || BigInt(0) }, BigInt(0))
  const reserved = accounts.reduce((current, account, idx) => { return current + BigInt(account.data ? account.data.reserved : 0) || BigInt(0) }, BigInt(0))
  const miscFrozen = 0n // accounts.reduce((current, account, idx) => { return current + BigInt(account.data ? account.data.miscFrozen : 0) || BigInt(0) }, BigInt(0))
  const feeFrozen = 0n // accounts.reduce((current, account, idx) => { return current + BigInt(account.data ? account.data.feeFrozen : 0) || BigInt(0) }, BigInt(0))
